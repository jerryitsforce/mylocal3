...
define([
    ...
    return function (config) {
        ...
        treeId.on('select_node.jstree', function (e, data) {
            _selectChildNodes(e, data);
        });
        treeId.on('deselect_node.jstree', function (e, data) {
            _deselectChildNodes(e, data);
        });
            ...
            if (response.length > 0) {
                response.forEach(function (newNode) {
                    ...
                    data.instance.create_node(childNode, newNode, 'last', function (node) {
                        var isParentSelected = data.instance.is_selected(childNode) && childNode.original && childNode.original.isManuallySelected;
                        if (checkedNodes.includes(node.id) || isParentSelected) {
        ...
        function _selectChildNodes (event, selected) {
            if (!firstInit) {
                if (selected.node && selected.node.original) {
                    selected.node.original.isManuallySelected = true;
                }
                // jscs:disable requireCamelCaseOrUpperCaseIdentifiers
                selected.instance.open_node(selected.node);
                if (selected.node.children && selected.node.children.forEach) {
        ...
        treeId.on('changed.jstree', function (e, data) {
            ...
            const clickedNodeID = data.node.id, currentCheckedNodes = data.selected.concat($(treeId).jstree('get_undetermined'));
