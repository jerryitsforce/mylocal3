...
define([
    ...
    'mage/dataPost',
    'mage/dropdowns'
], function ($, mageTemplate, urlValidator, alert, confirm, $t, customerData, DOMPurify) {
    'use strict';
    ...
    $.widget('mage.multipleWishlist', $.mage.multipleWishlist, {
        ...
        _moveItemTo: function (e) {
            var jQ = $.noConflict();
            var json = jQ(e.currentTarget).find('[data-wishlist-move-item]').data('wishlist-move-item'),
                tmplJson = {
                    qty: this._getQty(jQ(e.currentTarget)),
                    itemId: json.itemId,
                    url: this.moveWishlistJson.moveItemUrl,
                    'form_key': DOMPurify.sanitize(jQ('input[name="form_key"]').val()?.toString())
                },
                tmpl;
            ...
            if (json['new']) {
                ...
                tmpl = mageTemplate(this.options.formTmplSelector, {
                    data: tmplJson
                });
                jQ('body').append(DOMPurify.sanitize(tmpl));
                jQ(this.options.formTmplId).trigger('submit');
            }
        },
    ...
    $.widget('mage.multipleWishlist', $.mage.multipleWishlist, {
        ...
        _copyItemTo: function (e) {
            var jQ = $.noConflict();
            var json = jQ(e.currentTarget).find('[data-wishlist-copy-item]').data('wishlist-copy-item'),
                tmplJson = {
                    qty: this._getQty(jQ(e.currentTarget)),
                    itemId: json.itemId,
                ...
                actions: {
                    ...
                    confirm: function () {
                        ...
                        $.ajax({
                             ...
                             */
                            success: function () {
                                // Fix for Client DOM XSS: Strict Protocol Validation via DOM Parsing
                                var redirectUrl = json.redirectUrl;
                                if (redirectUrl) {
                                    var anchor = document.createElement('a');
                                    anchor.href = redirectUrl;
                                    if (anchor.protocol === 'http:' || anchor.protocol === 'https:' || (redirectUrl.startsWith('/') && !redirectUrl.startsWith('//'))) {
                                        if (/^https?:/.test(anchor.protocol)) { // Ensure parsed protocol is safe
    ...
    $.widget('mage.multipleWishlist', $.mage.multipleWishlist, {
        ...
        _addToNew: function (data) {
            ...
            this._callback = $.proxy(function (wishlistId) {
                ...
                $.ajax(url, {
                    method: 'POST',
                    data: $.extend(data.data, { form_key: $('input[name="form_key"]').val() }),
                    showLoader: true,
                    beforeSend: function () {
                    },
                    success: function () {
                        ...
                        $('.' + self.options.createTmplData.btnCloseClass).trigger('click');
        ...
        _buildWishlistDropdown: function () {
            ...
            if (this.options.wishlists && this.options.wishlists.length > 0) {
                $(this.options.wishlistLink).each($.proxy(function (index, e) {
                    var element = jQ(e),
                        buttonName = DOMPurify.sanitize(e.textContent.trim()),
                        ...
                        i, currentData, currentParams, tmpl;
                    ...
                    tmpl = mageTemplate(this.options.splitBtnTmpl, {
                        data: tmplData
                    })
                    jQ(element.parent()).prepend(DOMPurify.sanitize(tmpl));
    ...
    $.widget('mage.multipleWishlist', $.mage.multipleWishlist, {
        ...
        _create: function () {
            ...
            this.element.on('click', '[data-wishlist-to-giftregistry]', function () {
                ...
                $(DOMPurify.sanitize(mageTemplate(_this.options.formTmplSelector, {
                    data: tmplJson
                }))).appendTo('body');
