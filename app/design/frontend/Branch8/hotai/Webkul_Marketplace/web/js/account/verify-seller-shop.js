    ...
    $.widget('mage.verifySellerShop', {
        ...
        _create: function () {
            ...
            $(document).ready(function(){
                self.callAppendShopBlockFunction(this.element, 1);
            });
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response===0) {
                            ...
                            messageDiv.addClass('available message success').text(self.options.successMessage)
                            $(self.options.becomeSellerBoxWrapper).append(messageDiv);
                        } else {
                            $(self.options.button).addClass('disabled');
                            $(self.options.shopUrl).val('');
                            messageDiv.addClass('available message error').text(self.options.errorMessage)
                            $(self.options.becomeSellerBoxWrapper).append(messageDiv);
                        }
                    },
