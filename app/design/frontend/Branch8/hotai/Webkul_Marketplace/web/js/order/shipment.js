...
define([
    ...
    'mage/template',
    'plugins/DOMPurify'
], function ($, $t, alert, mageTemplate, DOMPurify) {
    'use strict';
    $.widget('mage.sellerOrderShipment', {
        _create: function () {
            ...
            jQ('#wk-mp-tracking-add').click(function (e) {
                jQ.ajax({
                    ...
                    data: {
                        ...
                        lpName: $('#wk-mp-tracking-lpname').val(),
                        lcUrl: $('#wk-mp-tracking-lcurl').val()
                    },
                    showLoader: true,
                    success:function ($data) {
                        if ($data.error) {
                            ...
                            jQ('#wk-mp-shipment-tracking-info-tbody').append(DOMPurify.sanitize(tmpl));
                            $('#wk-mp-tracking-carrier').val('custom');
                            $('#wk-mp-tracking-title').val('');
                            $('#wk-mp-tracking-number').val('');
                            $('#wk-mp-tracking-lcurl').val('');
                            $('#wk-mp-tracking-lpname').hide().val('');
                        }
                    },
                    error: function (response) {
                        alert({
