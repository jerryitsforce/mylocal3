...
class LogisticService extends AbstractHelper
{
     ...
     * @return bool
     */
    public function isEcpayHomeLogistics(?string $shippingMethod): bool
    {
     ...
     * @return bool
     */
    public function isEcpayCvsLogistics(?string $shippingMethod): bool
    {
    ...
    public function logisticCreateOrder(int $orderId)
    {
        ...
        $this->_loggerInterface->writeLog('LogisticService $shippingMethod:'.$shippingMethod);
        ...
        if ($this->isEcpayLogistics($shippingMethod)) {
            ...
            $this->_loggerInterface->writeLog('LogisticService goodsAmount:'. print_r($goodsAmount,true));

            // 收件人姓名
            $receiverName = $this->_orderService->getShippingName($orderId);
            $this->_loggerInterface->writeLog('LogisticService receiverName:'. print_r($receiverName,true));

            // 收件人電話
            $receiverCellPhone = $this->_orderService->getShippingTelephone($orderId);
            $this->_loggerInterface->writeLog('LogisticService receiverCellPhone:'. print_r($receiverCellPhone,true));

            // 貨到付款判斷
            $paymentMethod = $this->_orderService->getPaymentMethod($orderId);
            $this->_loggerInterface->writeLog('LogisticService paymentMethod:'.$paymentMethod);
            ...
            $this->_loggerInterface->writeLog('LogisticService logisticStage:'. print_r($logisticStage,true));

            // 取出CvsType
            $logisticCvsType = $this->_mainService->getLogisticConfig('logistic_cvs_type') ;
            $this->_loggerInterface->writeLog('LogisticService logisticCvsType:'. print_r($logisticCvsType,true));

            // 取出 URL
            $apiUrl = $this->getApiUrl('create', $logisticStage, $logisticCvsType);
            $this->_loggerInterface->writeLog('LogisticService apiUrl:'. print_r($apiUrl,true));
            ...
            if ($logisticStage == 1) {
                ...
                $this->_loggerInterface->writeLog('LogisticService accountInfo:'. print_r($accountInfo,true));
                ...
                $this->_loggerInterface->writeLog('LogisticService logisticMerchantId:'. print_r($logisticMerchantId,true));
                $this->_loggerInterface->writeLog('LogisticService logisticHashKey:'. print_r($logisticHashKey,true));
                $this->_loggerInterface->writeLog('LogisticService logisticHashIv:'. print_r($logisticHashIv,true));
            ...
            $this->_loggerInterface->writeLog('LogisticService logisticOrderPreFix:'. print_r($logisticOrderPreFix,true));

            // 組合廠商訂單編號
            $merchantTradeNo = $this->_orderService->getMerchantTradeNo($orderId, $logisticOrderPreFix);
            $this->_loggerInterface->writeLog('LogisticService merchantTradeNo:'. print_r($merchantTradeNo,true));

            // 取出寄件人姓名
            $logisticSenderName = $this->_mainService->getLogisticConfig('logistic_sender_name') ;
            $this->_loggerInterface->writeLog('LogisticService logisticSenderName:'. print_r($logisticSenderName,true));

            // 取出寄件人電話
            $logisticSenderCellphone = $this->_mainService->getLogisticConfig('logistic_sender_cellphone') ;
            $this->_loggerInterface->writeLog('LogisticService logisticSenderCellphone:'. print_r($logisticSenderCellphone,true));

            // 貨態回傳網址
            $serverReplyURL = $this->_urlInterface->getUrl('ecpaygeneral/Process/LogisticStatusResponse');
            $this->_loggerInterface->writeLog('LogisticService serverReplyURL:'. print_r($serverReplyURL,true));

            // 綠界訂單顯示商品名稱判斷

            $logisticDispItemName = $this->_mainService->getLogisticConfig('enabled_logistic_disp_item_name') ;
            $this->_loggerInterface->writeLog('LogisticService logisticDispItemName:'. print_r($logisticDispItemName,true));
            ...
            if ( $logisticDispItemName == 1 ) {
                ...
                $this->_loggerInterface->writeLog('LogisticService salesOrderItem:'. print_r($salesOrderItem,true));

                // 轉換商品名稱格式
                $itemName = $this->getItemName($salesOrderItem);
                $this->_loggerInterface->writeLog('LogisticService itemName:'. print_r($itemName,true));
            ...
            if ($this->isEcpayCvsLogistics($shippingMethod)) {
                ...
                $this->_loggerInterface->writeLog('LogisticService logisticsSubType:'. print_r($logisticsSubType,true));

                // 取出超商代碼
                $CVSStoreID = $this->_orderService->getEcpayLogisticCvsStoreId($orderId);
                $this->_loggerInterface->writeLog('LogisticService CVSStoreID:'.$CVSStoreID);

                $CVSStoreName = $this->_orderService->getEcpayLogisticCVSStoreName($orderId);
                $this->_loggerInterface->writeLog('LogisticService CVSStoreName:'.$CVSStoreName);
                ...
                $this->_loggerInterface->writeLog('LogisticService logisticSenderZipcode:'. print_r($logisticSenderZipcode,true));

                // 取出寄件人地址
                $logisticSenderAddress = $this->_mainService->getLogisticConfig('logistic_sender_address') ;
                $this->_loggerInterface->writeLog('LogisticService logisticSenderAddress:'. print_r($logisticSenderAddress,true));

                // 收件人郵遞區號
                $receiverZipCode = $this->_orderService->getShippingPostcode($orderId);
                $this->_loggerInterface->writeLog('LogisticService receiverZipCode:'. print_r($receiverZipCode,true));

                // 收件人地址
                $receiverStreet = $this->_orderService->getShippingStreet($orderId);
                $this->_loggerInterface->writeLog('LogisticService receiverStreet:'. print_r($receiverStreet,true));
                ...
                $this->_loggerInterface->writeLog('LogisticService collectRates goodsWeight:'. print_r($goodsWeight,true) . ' ' . $weightUnit);

                // 單位轉換
                if ($weightUnit == 'lbs') {

                    $goodsWeight = $goodsWeight * 0.45359;
                    $this->_loggerInterface->writeLog('LogisticService collectRates goodsWeight:'. print_r($goodsWeight,true) . 'KG');
                }
            ...
            $this->_loggerInterface->writeLog('LogisticService inputLogisticOrder:'. print_r($inputLogisticOrder,true));
            $this->_loggerInterface->writeLog('LogisticService response:'. print_r($response,true));
