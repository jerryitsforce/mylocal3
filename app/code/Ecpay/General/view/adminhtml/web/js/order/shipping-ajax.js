        ...
        var shippingFunction = {
                        ...
                        if (response.code == '0999') {
                            switch (method) {
                                ...
                                case 'print':
                                    ...
                                    $('#print_form').empty();
                                    jQ('#print_form').append(DOMPurify.sanitize(data.form_print));
                                    $('#ecpay_print').submit();
                                    break; 
                                case 'store':
                                    var data = JSON.parse(response.data);
                                    ...
                                    jQ('#map_form').append(DOMPurify.sanitize(form));
                                    $('#ecpay-form').submit();
                                    break;
                            }
                        }
            ...
            appendShippingData: function(shipping_data) {
                ...
                if(shipping_data.logistics_type == 'CVS' || (shipping_data.cvs_store_id != undefined && shipping_data.cvs_store_id != null)) {
                    ...
                    //超商編號
                    jQ('#csv_info').append('<label>' + $t('CVS Store No：') + '</label><span id="ecpay_cvs_store_id">' + (shipping_data.cvs_store_id == null ? '' : DOMPurify.sanitize(shipping_data.cvs_store_id)) + '</span><br>');
                    //超商名稱
                    jQ('#csv_info').append('<label>' + $t('CVS Store Name：') + '</label></label><span id="ecpay_cvs_store_name">' + (shipping_data.cvs_store_name == null ? '' : DOMPurify.sanitize(shipping_data.cvs_store_name)) + '</span><br>');
                }
            }
        }
