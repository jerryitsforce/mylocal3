...
class OrderShipping extends AbstractOrder
{
    ...
    public function __construct(
        ...
        $this->_loggerInterface->writeLog('OrderShipping orderId:'. print_r($this->orderId, true));
    }
    ...
    public function getShippingData()
    {
        ...
        $this->_loggerInterface->writeLog('OrderShipping $shippingMethod:'. print_r($shippingMethod, true));
        ...
        if ($this->_logisticService->isEcpayLogistics($shippingMethod)) {
            ...
            $this->_loggerInterface->writeLog('OrderShipping $ecpayShippingTag:'. print_r($ecpayShippingTag, true));

            // 已開立物流訂單
            if ($ecpayShippingTag === '1') {
                // 取得綠界物流單資訊
                $ecpayLogisticInfo = $this->_orderService->getEcpayLogisticInfo($this->orderId);
                $this->_loggerInterface->writeLog('OrderShipping $ecpayLogisticInfo:'. print_r($ecpayLogisticInfo, true));
                ...
                // 判斷物流種類
                $this->_loggerInterface->writeLog('OrderShipping $isEcpayCvsLogistics:'. print_r($this->_logisticService->isEcpayCvsLogistics($shippingMethod), true));
            ...
            $this->_loggerInterface->writeLog('OrderShipping $extension:'. print_r($extension, true));
