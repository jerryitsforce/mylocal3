...
class PaymentToEcpay extends Action implements CsrfAwareActionInterface
{
     ...
     */
    private PurifierInterface $_purifier;
    ...
    public function execute()
    {
        ...
        $this->_loggerInterface->writeLog('PaymentToEcpay paymentStage:'. print_r($paymentStage,true));

        // 取出 URL
        $apiUrl = $this->_paymentService->getApiUrl('check_out', $paymentStage);
        $this->_loggerInterface->writeLog('PaymentToEcpay apiUrl:'. print_r($apiUrl,true));
        ...
        if ($paymentStage == 1) {
            ...
            $this->_loggerInterface->writeLog('PaymentToEcpay accountInfo:'. print_r($accountInfo,true));
            ...
            $this->_loggerInterface->writeLog('PaymentToEcpay paymentMerchantId:'. print_r($paymentMerchantId,true));
            $this->_loggerInterface->writeLog('PaymentToEcpay paymentHashKey:'. print_r($paymentHashKey,true));
            $this->_loggerInterface->writeLog('PaymentToEcpay paymentHashIv:'. print_r($paymentHashIv,true));
        ...
        $this->_loggerInterface->writeLog('PaymentToEcpay enctyOrderId:'. print_r($enctyOrderId,true));
        $this->_loggerInterface->writeLog('PaymentToEcpay orderId:'. print_r($orderId,true));

        // 取出訂單金流方式
        $paymentMethod = $this->_orderService->getPaymentMethod($orderId);
        $this->_loggerInterface->writeLog('PaymentToEcpay paymentMethod:'. print_r($paymentMethod,true));

        // 取出訂單前綴
        $paymentOrderPreFix = $this->_mainService->getPaymentConfig('payment_order_prefix');
        $this->_loggerInterface->writeLog('PaymentToEcpay paymentOrderPreFix:'. print_r($paymentOrderPreFix,true));

        // 組合廠商訂單編號
        $merchantTradeNo = $this->_orderService->getMerchantTradeNo($orderId, $paymentOrderPreFix);
        $this->_loggerInterface->writeLog('PaymentToEcpay merchantTradeNo:'. print_r($merchantTradeNo,true));

        // 訂單金額
        $totalAmount = (int) ceil($this->_orderService->getGrandTotal($orderId));
        $this->_loggerInterface->writeLog('PaymentToEcpay $totalAmount:'. print_r($totalAmount,true));

        // 綠界訂單是否顯示商品名稱
        $paymentDispItemName = $this->_mainService->getPaymentConfig('enabled_payment_disp_item_name');
        $this->_loggerInterface->writeLog('PaymentToEcpay paymentDispItemName:'. print_r($paymentDispItemName,true));
        ...
        if ( $paymentDispItemName == 1 ) {
            ...
            $this->_loggerInterface->writeLog('PaymentToEcpay salesOrderItem:'. print_r($salesOrderItem,true));

            // 轉換商品名稱格式
            $itemName = $this->_paymentService->convertToPaymentItemName($salesOrderItem);
            $this->_loggerInterface->writeLog('PaymentToEcpay itemName:'. print_r($itemName,true));
        ...
        $this->_loggerInterface->writeLog('PaymentToEcpay $paymentInfoURL:'. print_r($paymentInfoURL,true));

        // 回傳資訊網址 - ReturnURL
        $returnURL = $this->_urlInterface->getUrl('ecpaygeneral/Process/PaymentResponse');
        $returnURL = $returnURL . '?id='. $enctyOrderId;
        $this->_loggerInterface->writeLog('PaymentToEcpay $returnURL:'. print_r($returnURL,true));

        $clientBackURL = $this->_urlInterface->getUrl('ecpaygeneral/Page/ThankYou');
        $clientBackURL = $clientBackURL . '?id='. $enctyOrderId;
        $this->_loggerInterface->writeLog('PaymentToEcpay $clientBackURL:'. print_r($clientBackURL,true));
        ...
        $this->_loggerInterface->writeLog('PaymentToEcpay $additionalInformation:'. print_r($additionalInformation,true));
        ...
        echo $this->_purifier->purify($this->_paymentService->checkout($accountInfo, $input, $apiUrl));
