...
class PaymentInfoResponse extends Action implements CsrfAwareActionInterface
{
    ...
    public function execute()
    {
        ...
        $this->_loggerInterface->writeLog('PaymentInfoResponse paymentInfo:'. print_r($paymentInfo,true));
        ...
        if (count($paymentInfo) < 1) {
            ...
            $this->_loggerInterface->writeLog('PaymentInfoResponse orderInfo:'. print_r($orderInfo,true));
            ...
            $this->_loggerInterface->writeLog('PaymentInfoResponse orderId:'. print_r($orderId, true));
            ...
            if (!$this->_paymentService->validAmount($paymentInfo['TradeAmt'], $orderTotal) || !$checkMacValue) {
                $this->_loggerInterface->writeLog('PaymentInfoResponse $paymentInfo.TradeAmt:'. print_r($paymentInfo['TradeAmt'], true));
                $this->_loggerInterface->writeLog('PaymentInfoResponse $orderTotal:'. print_r($orderTotal, true));
                ...
                $this->_loggerInterface->writeLog('PaymentInfoResponse $orderStatus:'. print_r($orderStatus, true));

                if ($orderStatus === $createStatus) {
                    $responseResult = $this->_paymentService->getPaymentInfoResponse($orderId, $paymentInfo);
                    $this->_loggerInterface->writeLog('PaymentInfoResponse $responseResult:'. print_r($responseResult, true));
                    ...
                    if ($responseResult['status'] === 1) {
                        ...
                        if ($paymentType != 'BNPL') {
                            ...
                            $this->_loggerInterface->writeLog('PaymentInfoResponse $templateValues:'. print_r($templateValues, true));
