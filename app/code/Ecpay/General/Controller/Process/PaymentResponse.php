...
class PaymentResponse extends Action implements CsrfAwareActionInterface
{
    ...
    public function execute()
    {
        ...
        $this->_loggerInterface->writeLog('PaymentResponse paymentInfo:'. print_r($paymentInfo,true));
        ...
        if (count($paymentInfo) < 1) {
            ...
            $this->_loggerInterface->writeLog('PaymentResponse orderInfo:'. print_r($orderInfo,true));
            ...
            $this->_loggerInterface->writeLog('PaymentResponse orderId:'. print_r($orderId, true));
            ...
            if (!$this->_paymentService->validAmount($paymentInfo['TradeAmt'], $orderTotal) || !$checkMacValue) {
                $this->_loggerInterface->writeLog('PaymentResponse $paymentInfo.TradeAmt:'. print_r($paymentInfo['TradeAmt'], true));
                $this->_loggerInterface->writeLog('PaymentResponse $orderTotal:'. print_r($orderTotal, true));
                ...
                $this->_loggerInterface->writeLog('PaymentResponse $orderStatus:'. print_r($orderStatus, true));
                ...
                if (in_array($orderStatus, $createStatus) && !$paymentCompleteFlag) {
                    ...
                    if (isset($paymentInfo['SimulatePaid']) && intval($paymentInfo['SimulatePaid']) === 0){
                        ...
                        $this->_loggerInterface->writeLog('PaymentResponse $responseResult:'. print_r($responseResult, true));
