...
class LogisticMapToEcpay extends Action implements CsrfAwareActionInterface
{
    ...
    public function execute()
    {
        ...
        $this->_loggerInterface->writeLog('LogisticMapToEcpay logisticStage:'. print_r($logisticStage,true));

        // 取出CvsType
        $logisticCvsType = $this->_mainService->getLogisticConfig('logistic_cvs_type') ;
        $this->_loggerInterface->writeLog('LogisticMapToEcpay logisticCvsType:'. print_r($logisticCvsType,true));

        // 取出 URL
        $apiUrl = $this->_logisticService->getApiUrl('map', $logisticStage, $logisticCvsType);
        $this->_loggerInterface->writeLog('LogisticMapToEcpay apiUrl:'. print_r($apiUrl,true));
        ...
        if ($logisticStage == 1){
            ...
            $this->_loggerInterface->writeLog('LogisticMapToEcpay accountInfo:'. print_r($accountInfo,true));
            ...
            $this->_loggerInterface->writeLog('LogisticMapToEcpay logisticMerchantId:'. print_r($logisticMerchantId,true));
            $this->_loggerInterface->writeLog('LogisticMapToEcpay logisticHashKey:'. print_r($logisticHashKey,true));
            $this->_loggerInterface->writeLog('LogisticMapToEcpay logisticHashIv:'. print_r($logisticHashIv,true));
        ...
        $this->_loggerInterface->writeLog('LogisticMapToEcpay enctyOrderId:'. print_r($enctyOrderId,true));
        $this->_loggerInterface->writeLog('LogisticMapToEcpay orderId:'. print_r($orderId,true));

        // 取出訂單物流方式
        $shippingMethod = $this->_orderService->getShippingMethod($orderId);
        $this->_loggerInterface->writeLog('LogisticMapToEcpay shippingMethod:'. print_r($shippingMethod,true));

        // 取出訂單前綴
        $logisticOrderPreFix = $this->_mainService->getLogisticConfig('logistic_order_prefix') ;
        $this->_loggerInterface->writeLog('LogisticMapToEcpay logisticOrderPreFix:'. print_r($logisticOrderPreFix,true));

        // 組合廠商訂單編號
        $merchantTradeNo = $this->_orderService->getMerchantTradeNo($orderId, $logisticOrderPreFix);
        $this->_loggerInterface->writeLog('LogisticMapToEcpay merchantTradeNo:'. print_r($merchantTradeNo,true));

        // 取出物流子類型
        $logisticsSubType = $this->_logisticService->getCvsLogisticsSubType($logisticCvsType, $shippingMethod);
        $this->_loggerInterface->writeLog('LogisticMapToEcpay logisticsSubType:'. print_r($logisticsSubType,true));

        // 取出訂單金流方式
        $paymentMethod = $this->_orderService->getPaymentMethod($orderId);
        $this->_loggerInterface->writeLog('LogisticMapToEcpay paymentMethod:'. print_r($paymentMethod,true));
        ...
        $this->_loggerInterface->writeLog('MapToEcpay $serverReplyURL:'. print_r($serverReplyURL,true));
