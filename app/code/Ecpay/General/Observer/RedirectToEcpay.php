...
class RedirectToEcpay implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        $this->_loggerInterface->writeLog('RedirectProcess Event $result:');

        // logging to test overrid
        $this->_loggerInterface->writeLog('RedirectProcess Event $orderId:'. print_r($orderId,true));

        // 取出物流方式
        $shippingMethod = $order->getShippingMethod();
        $this->_loggerInterface->writeLog('RedirectProcess Event $shippingMethod:'.$shippingMethod);

        // 取出金流方式
        $payment = $order->getPayment();
        $paymentMethod = $payment->getMethod();
        $this->_loggerInterface->writeLog('RedirectProcess Event $paymentMethod:'.$paymentMethod);

         // 訂單編號加密
        $encOrderId = $this->_encryptionsService->encrypt($orderId);
        $this->_loggerInterface->writeLog('RedirectProcess Event $orderId:'. print_r($orderId,true));
        $this->_loggerInterface->writeLog('RedirectProcess Event $encOrderId:'. print_r($encOrderId,true));
        ...
        if ($isEcpayLogistic) {
            ...
            if ($cvsOrHomeCheck == 'cvs') {
                ...
                $this->_loggerInterface->writeLog('RedirectProcess Event $redirectUrl:'. print_r($redirectUrl,true));
            ...
            if ($isEcpayPyment) {
                ...
                $this->_loggerInterface->writeLog('RedirectProcess Event $invoiceType:'. print_r($invoiceType,true));
                ...
                if (in_array($invoiceType, [EcpayInvoice::ECPAY_INVOICE_TYPE_C, EcpayInvoice::ECPAY_INVOICE_TYPE_D, EcpayInvoice::ECPAY_INVOICE_TYPE_P])) {
                    ...
                    $this->_loggerInterface->writeLog('RedirectProcess ecpayEnableInvoice:'. print_r($ecpayEnableInvoice,true));

                    // 判斷發票是否啟用自動開立
                    $ecpayInvoiceAuto = $this->_mainService->getInvoiceConfig('enabled_invoice_auto') ;
                    $this->_loggerInterface->writeLog('RedirectProcess ecpayInvoiceAuto:'. print_r($ecpayInvoiceAuto,true));
