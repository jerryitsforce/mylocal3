        ...
        var invoiceFunction = {
                ...
                jQ.ajax({
                    ...
                    error: function(response) {
                        ...
                        if (response.code == '1005') {
                            invoiceFunction.showMessage('success', '0元商品不需開立發票')
                        } else if (response.code == '1004' || response.code == '1003' || response.code == '1002') {
            ...
            appendInvoiceData: function(invoice_data) {
                ...
                if (invoice_data.ecpay_invoice_love_code !== undefined && invoice_data.ecpay_invoice_love_code !== null) 
                    jQ('#invoice_codes').append('<label>' + $t('Donation Code') + '：</label><span id="invoice_love_code"> ' + DOMPurify.sanitize(invoice_data.ecpay_invoice_love_code) + '</span><br>')
                // 公司行號
                if (invoice_data.ecpay_invoice_customer_company !== undefined && invoice_data.ecpay_invoice_customer_company !== null) 
                    jQ('#invoice_codes').append('<label>' + $t('Company Name') + '：</label><span id="invoice_customer_company"> ' + DOMPurify.sanitize(invoice_data.ecpay_invoice_customer_company) + '</span><br>')
                //統一編號
                if (invoice_data.ecpay_invoice_customer_identifier !== undefined && invoice_data.ecpay_invoice_customer_identifier !== null) 
                    jQ('#invoice_codes').append('<label>' + $t('Uniform Numbers') + '：</label><span id="invoice_customer_identifier"> ' + DOMPurify.sanitize(invoice_data.ecpay_invoice_customer_identifier) + '</span><br>')
                //載具編號
                if (invoice_data.ecpay_invoice_carruer_num !== undefined && invoice_data.ecpay_invoice_carruer_num !== null) 
                    jQ('#invoice_codes').append('<label>' + $t('Carrier Code') + '：</label><span id="invoice_carruer_num"> ' + DOMPurify.sanitize(invoice_data.ecpay_invoice_carruer_num) + '</span><br>')
            }
        }
