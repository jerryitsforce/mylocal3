...
abstract class AbstractGiftAction implements Discount\DiscountInterface
{
    ...
    public function calculate($rule, $item, $qty)
    {
        ...
        foreach ($skus as $sku)
        {
            try
            {
                $product = $this->getGiftProduct($sku);
                if ($rule->getSimpleAction() == 'add_gift') {
                    if($item->getQuote()->hasProductId($product->getId())) continue;
                }
                $giftQty = $this->getGiftQty($item, $rule, $qty);
                $quoteItem = $item->getQuote()->addProduct($product, $giftQty);
