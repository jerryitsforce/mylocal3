...
class Processor
{
    ...
    public function __construct(
        ...
        if (strpos($this->_reportDir, BP) !== 0) {
        ...
        if (!empty($this->request->getParam('SCRIPT_NAME'))) {
            $rawScriptName = $this->request->getParam('SCRIPT_NAME');
            // Sanitize: remove path traversal sequences and normalize separators
            $rawScriptName = str_replace('\\', '/', $rawScriptName);
            $rawScriptName = preg_replace('#(\.\./|\./)+#', '', $rawScriptName);
            if (in_array(basename($rawScriptName, '.php'), ['404', '503', 'report'])) {
                $this->_scriptName = dirname($rawScriptName);
            } else {
                $this->_scriptName = $rawScriptName;
            }
        }
    ...
    protected function _setReportData($reportData)
    {
        ...
        if (isset($this->reportData['script_name'])) {
            // Sanitize stored script_name to prevent path traversal via report data
            $storedScriptName = str_replace('\\', '/', (string) $this->reportData['script_name']);
            $storedScriptName = preg_replace('#(\.\./|\./)+#', '', $storedScriptName);
            $this->_scriptName = $storedScriptName;
        }
    }
    ...
    public function loadReport($reportId)
    {
        try {
            ...
            // Guard: ensure resolved file path is strictly within the allowed report directory
            if (strpos($this->_reportFile, $this->_reportDir) !== 0) {
                return;
            }
    ...
    private function getReportPath(int $reportDirNestingLevel, string $reportId): string
    {
        ...
        $fullPath = $reportDirPath . $reportId;
        if (strpos($fullPath, $this->_reportDir) !== 0) {
    ...
    private function getReportDirNestingLevel(string $reportId): int
    {
        ...
        $value   = $this->request->getParam($envName);
        if ($value !== null) {
            $value = (int)$value;
            if ($value < 0 || $value > 3) {
                 $value = 0;
