...
define([
    ...
    $.widget('mageplaza.mpProcessBar', {
        ...
        getContent: function (percent, itemError, status) {
            ...
            // Fix for Client DOM XSS: Sanitize inputs before construction
            // Using a simple text node approach where possible, or explicit sanitization if HTML is needed.
            // Here status and itemError are treated as potentially unsafe.

            // Safe construction using text nodes for dynamic parts requires breaking up the HTML string
            // OR sanitizing the strings first if they might contain HTML (e.g. bold tags are added in JS, so we assume inputs are text)

            // Ideally we use DOMPurify if available, but it might not be imported in this old file.
            // Checking imports... 'jquery', 'Magento_Ui/js/modal/confirm', 'alert', 'modal'. DOMPurify IS NOT imported. 
            // We should text-encode.
            ...
            strong.textContent = status; // Safe sink

            p.appendChild(strong);
            p.appendChild(document.createTextNode(': ' + itemError)); // Safe sink
