...
class BundlesLoader extends Template
{
    ...
    protected function _addBundleConfig($bundleConfigPath) {
        ...
        $mixinsKey = $this->requireJsConfig->getMixinsFileRelativePath();

        if ($assetCollection->has($mixinsKey)) {
            ...
            $assetCollection->add($bundleConfigRelPath, $bundleConfigAsset);
        }
    }
    ...
    protected function _getBundlesConfigPaths() {
        ...
        if ($commonBundle) {
            $configPaths[] = $this->_getBestConfigPath($commonBundle['config_path']);
        }

        $pageBundles = $this->getPageBundles() ?? [];
        foreach ($pageBundles as $pageBundle) {
            $configPaths[] = $this->_getBestConfigPath($pageBundle['config_path']);
        }

        return array_reverse(array_filter($configPaths));
    }

    /**
     * Tries to find the best configuration path (minified or not) based on file existence.
     */
    protected function _getBestConfigPath($path) {
        if (!$path) {
            return null;
        }

        if ($this->minification->isEnabled('js')) {
            return $this->minification->addMinifiedSign($path);
        }
