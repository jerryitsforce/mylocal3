...
class Faq extends \Magento\Framework\View\Element\Template
{
    ...
    public function getFrequentlyAskedQuestionsByCategory($categoryId)
    {
        // Build the query to fetch FAQs based on the given category ID
        $select = $this->_faqResourceModel->getConnection()->select()
            ->from(['faq' => $this->_faqResourceModel->getMainTable()])  // FAQ table
            ->joinLeft(
                ['faq_store' => $this->_faqResourceModel->getTable('phpcuong_faq_store')],
                'faq.faq_id = faq_store.faq_id',
                ['store_id']  // Join to get the store ID
            )
            ->joinLeft(
                ['faq_category' => $this->_faqResourceModel->getTable('phpcuong_faq_category_id')],
                'faq.faq_id = faq_category.faq_id',
                []  // Joining the FAQ category relation table
            )
            ->where('faq_store.store_id = ?', $this->_storeManager->getStore()->getStoreId())  // Match store ID
            ->where('faq.is_active = ?', '1')  // Only active FAQs
            ->where('faq_category.category_id = ?', $categoryId)  // Filter by the given category ID
            ->order('faq.sort_order ASC');  // Sort FAQs based on some order

        // Fetch and return results
        $results = $this->_faqResourceModel->getConnection()->fetchAll($select);

        // Return the FAQs or false if none found
