/**
 * HotaiConnected CheckoutManagement - 例外授權 Tabulator 配置
 * 
 * 此檔案包含例外授權表格的業務特定配置（當 data_source = '2' 時使用）
 * 基於 UiShared 的 getRemoteConfig() 遠端分頁框架
 * 
 * @module HotaiConnected_CheckoutManagement/js/checkoutarea/config/exception-auth-config
 * @version 1.0.0
 * @author HotaiConnected
 * @created 2025-12-09
 * @updated 2025-12-09 - 新增：獨立例外授權配置，方便抽換
 * 
 * ========================================
 * 配置繼承關係
 * ========================================
 ...
 * 
 * ExceptionAuthConfig (本模組)     ← 覆蓋業務特定配置
 * ├── ajaxURL: /rest/V1/reconciliation/exception-auth
 ...
 * 
 * 1. ajaxURL
 *    - API 端點：/rest/V1/reconciliation/exception-auth
 * 
 * 2. paginationSize
 ...
 * 
 * @example
 * // 在 index.js 中使用（當 data_source = '2' 時）
 * define([
 *     'tabulator',
 *     'HotaiConnected_CheckoutManagement/js/checkoutarea/config/exception-auth-config'
 * ], function(Tabulator, exceptionAuthConfig) {
 *     var config = exceptionAuthConfig.getConfig();
...
define([
    ...
    return {
         ...
         * 
         * 使用方式：
         * - apply-filters 時：exceptionAuthConfig.currentFilterParams = params || {};
         * - clear-filters 時：exceptionAuthConfig.currentFilterParams = {};
         * - ajaxURLGenerator 中：var filterParams = self.currentFilterParams || {};
         */
        currentFilterParams: {},
        
        /**
         * 轉換例外授權資料項目（統一資料轉換邏輯，消除重複代碼）
         * @private
         * 
         * 此函數為純粹的資料轉換函數，只負責：
        ...
        getConfig: function(options) {
            ...
            var businessConfig = {
                ...
                // 0. 禁用 row header（移除 checkbox）
                rowHeader: false,
                
                // 1. AJAX 載入資料的 URL (例外授權 API)
