...
define([
...
], function ($, $t, validation, domReady, searchableDialogMultiselect, datePickers, toastMessage, filtersBase, api) {
    'use strict';
    ...
    return {
        ...
        // 表單選擇器（用於 filtersBase）
        selectors: {
            form: '#create_monthly_batch_form'
        },
        ...
        initStatusData: function() {
            ...
            api.getReconciliationStatusData().done(function(response) {
                ...
                // 初始化商品物流狀態（使用 SumoSelect）
                self.initOrderStatus(response.order_status);
                // 初始化發票狀態（使用 SumoSelect）
                self.initInvoiceStatus(response.invoice_status);
                // 初始化票券狀態（使用 SumoSelect）
                self.initTicketStatus(response.ticket_status);
            }).fail(function(response) {
                console.error('initStatusData 失敗', response);
                $('[name="product-shipping-status"], [name="invoice-status"], [name="ticket-status"]').closest('[data-type="multiselect"]').addClass('fail');
            });
        },
        ...
        initOrderStatus: function(data) {
            ...
            // 轉換資料格式為 SumoSelect 需要的格式 [{ id, text }]
            var orderStatusOptions = [];
            data.forEach(function(status) {
                var displayName = $.mage.__(status);  // 使用 Magento 翻譯功能
            ...
            // 使用 filtersBase 的 updateSelectData 方法更新 SumoSelect
            filtersBase.updateSelectData.call(self, 'product-shipping-status', orderStatusOptions, { selectAll: true });
            
            self.componentInitFlags.initOrderStatus = true;
            $('.product-shipping').addClass('active');
        ...
        initInvoiceStatus: function(data) {
            ...
            data.forEach(function(item) {
                var displayName = item.zh_name || item.en_name;
                invoiceStatusOptions.push({
                    id: item.code,
                    text: displayName
                });
            });
            ...
            // 使用 filtersBase 的 updateSelectData 方法更新 SumoSelect
            filtersBase.updateSelectData.call(self, 'invoice-status', invoiceStatusOptions, { selectAll: true });
            
            self.componentInitFlags.initInvoiceStatus = true;
        ...
        initTicketStatus: function(data) {
            ...
            // 使用 filtersBase 的 updateSelectData 方法更新 SumoSelect
            filtersBase.updateSelectData.call(self, 'ticket-status', ticketStatusOptions, { selectAll: true });
            
            self.componentInitFlags.initTicketStatus = true;
            ...
            api.getReconciliationSellers().done(function(data) {
                ...
                data.items.forEach(function(item) {
                    ...
                    authorizedDealerOptions[item.seller_id] = {
                        key: item.seller_code,
                        value: item.shop_title || self.defaultValues.text,
                    };
                });

                self.data.authorizedDealer = authorizedDealerOptions;

                // validationMessage = validationMessage.replace('%1', $.mage.__('Authorized Dealer Name'));
        ...
        initFormValidation: function() {
            ...
            self._form.validation({
                ...
                errorPlacement: function(error, element) {
                    ...
                    // 3. 處理 SumoSelect 多選下拉選單
                    else if (element.is('select[multiple]')) {
                        // SumoSelect 會將原始的 select 包裝在 .SumoSelect 容器中
                        // 需要先找到 select，然後往上找父容器 .SumoSelect
                        var $select = $(element);
                        // SumoSelect 會將 select 包裝在父容器中，所以往上找 .SumoSelect
                        var $sumoSelect = $select.parent('.SumoSelect');
                        
                        if ($sumoSelect.length > 0) {
                            // 如果找到 SumoSelect 容器，將錯誤訊息放在容器下方
                            error.insertAfter($sumoSelect);
                        } else {
                            // 如果還沒初始化 SumoSelect，放在 select 的父容器下方
                            error.insertAfter($select.closest('.admin__field-control'));
                        }
                    }
                    // 4. 其他所有欄位
                    else {
                ...
                success: function(label, element) {
                    ...
                    // 移除元件的錯誤狀態
                    if ($element.closest('.searchable-dialog-multiselect__display').length > 0) {
        ...
        getFormData: function() {
            ...
            // 從 SumoSelect 取得選中的值（返回陣列）
            var productShippingStatus = $('[name="product-shipping-status"]').val() || [];
            var invoiceStatus = $('[name="invoice-status"]').val() || [];
            var ticketStatus = $('[name="ticket-status"]').val() || [];
            ...
            var exclude_pending_gift_order = $('[name="exclude-unconfirmed-gift-orders"]').is(':checked') ? 1 : 0;
