...
{
    const LOG_FOLDER_NAME = 'Qware/Cron/TicketRequestRetry';
    ...
    public function execute()
    {
        $this->hotaiCoreCommonHelper->writeLog(json_encode([
            "Message" => "QwareTicketRequestRetry cron start.",
        ...
        foreach ($itemObjArray as $item) {
            ...
            try {
                ...
                $this->qwareTicketRecordRepository->storeVoucherDataInDatabase($apiResponse, $item, $customOwner);
                
                // 檢查是否為 202 狀態
                if (isset($apiResponse['Code']) && $apiResponse['Code'] == 202) {
                    $this->hotaiCoreCommonHelper->writeLog(json_encode([
                        "Order ID"      => $item->getOrderId(),
                        "Order item ID" => $item->getId(),
                        "Message"       => "API returned 202, order is pending. Item status updated to PENDING."
                    ]), self::LOG_FOLDER_NAME);
                    
                    // 202 狀態已在 storeVoucherDataInDatabase 中處理，無需進一步處理
                    continue;
                }
        ...
        $this->hotaiCoreCommonHelper->writeLog(json_encode([
            "Message" => "QwareTicketRequestRetry cron end.",
        ]), self::LOG_FOLDER_NAME);
    }
    ...
    {
        $qwareCollection = $this->qwareCollectionFactory->create();
        $qwareCollection->addFieldToSelect([
            QwareTicketRecord::RECORD_ID,
            QwareTicketRecord::SALES_ORDER_ITEM_ID,
        ]);
        $qwareCollection->addFieldToFilter(QwareTicketRecord::SALES_ORDER_ITEM_ID, $item->getId());
        $qwareCollection->addFieldToFilter(QwareTicketRecord::STATUS, TicketStatus::STATUS_UNUSED);

        if ($qwareCollection->getSize() > 0) {
            return true;
        }
        // -----------------
        ...
        if ($newCount >= self::RETRY_COUNT_LIMIT) {
            ...
            try {
                $this->emailNotificationService->sendCronRetryFailedNotification(
                    'TicketRequestRetry',
                    $item->getId(),
                    'Ticket request retry failed after ' . self::RETRY_COUNT_LIMIT . ' attempts'
                );
            } catch (\Exception $e) {
                // 記錄郵件發送失敗，但不影響主流程
                $this->hotaiCoreCommonHelper->writeLog(json_encode([
                    'Title' => 'Failed to send email notification for TicketRequestRetry',
    ...
    protected function resolveCustomOwnerForGiftOrder(OrderItem $item): ?array
    {
        try {
            ...
            return [
                'telephone' => $telephone,
                'customer_id' => 0  // Will be resolved in QwareTicketRecordRepository
            ];
