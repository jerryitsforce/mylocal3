 ...
 * @updated 2025-01-26 - 新增 isAllSelected() 和 getSelectionInfo() 方法（全選判斷）
 ...
 * - 支援搜尋、多選、Checkbox 功能
 * - 精簡的 API：init(), updateData(), resetSelection(), getValue(), isAllSelected(), getSelectionInfo()
 * 
 * 核心需求：
 * 1. 輸入框只顯示已選的 tags
 * 2. 下拉選單與輸入框緊密貼合
...
define([
    ...
    return {
        ...
        /**
         * 取得選中資訊（選中數量、總數量、選中值陣列）
         * @param {string} selector - 選擇器
         * @returns {Object|null} { selectedCount: number, totalCount: number, selectedValues: Array, isEmpty: boolean }
         */
        getSelectionInfo: function(selector) {
            var $select = $(selector);
            if ($select.length === 0) {
                console.error('SumoSelect Util (getSelectionInfo): Element not found - ' + selector);
                return null;
            }
         ...
         * @example
         * // 判斷是否全選
         * if (sumoUtil.isAllSelected('#my-select')) {
         *     console.log('已全選');
         * }
         */
        isAllSelected: function(selector) {
            var info = this.getSelectionInfo(selector);
            if (info === null) {
                return null;
            }
            return info.selectedCount === info.totalCount && info.totalCount > 0;
        },
