...
class Data extends AbstractHelper
{
    ...
    public function checkNotificationUpdate()
    {
        ...
        $curl->write(Request::METHOD_GET, Data::getUpdateNotificationUrl() . $param);
        $data = $curl->read();
            ...
            if ($data !== null) { // Check if $data is not null
                ...
                if ($count) {
                    foreach ($data as $value) {
                        $this->addNotification(@$value['id'], @$value['severity'], @$value['created_at'],
                            @$value['title'], @$value['description'], @$value['url']);
                    }
                }
            }
        }
