...
class VariationCost extends Column
{
    ...
    {
        if ($this->scopeConfig->isSetFlag('branch8_catalog/product_approval_setting/hide_variation_cost')) {
    ...
    public function prepareDataSource(array $dataSource): array
    {
        if (empty($dataSource['data']['items'])
            || $this->scopeConfig->isSetFlag('branch8_catalog/product_approval_setting/hide_variation_cost')) {
            return $dataSource;
        }
        ...
        /*Get Variant Cost from Changes if it has*/
                ...
                if (isset($dataInformation['wk_manage_variation'])) {
                    ...
                    try {
                        ...
                        foreach ($collection->getItems() as $variant) {
                            $price = $variant->getData('cost');
                            $title = 'Cost';
                            $variantValue[] = sprintf(
                                '<span><strong style="white-space: nowrap;">%s</strong> : <span>%s</span></span>',
                                $variant->getData('comb'),
                                $variant->getData('cost')
                            );
                        }
                        $item[$fieldName] = '<div style="width: 100px;">'.implode('<br>', $variantValue).'</div>';
                    } catch (\Exception $e) {
