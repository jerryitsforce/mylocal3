...
class AdminhtmlCustomerSaveAfterObserver implements ObserverInterface
{
    ...
    public function __construct(
        ...
        \Magento\Customer\Model\CustomerFactory $customerFactory = null,
        MpHelper $mpHelper = null,
        MpEmailHelper $mpEmailHelper = null,
        \Webkul\Marketplace\Model\ProductFactory $mpProductFactory = null,
        \Magento\Framework\Module\Dir\Reader $reader = null,
        ...
        $this->customerFactory = $customerFactory ?: \Magento\Framework\App\ObjectManager::getInstance()
        ...
        $this->mpProductFactory = $mpProductFactory ?: \Magento\Framework\App\ObjectManager::getInstance()
            ->create(\Webkul\Marketplace\Model\ProductFactory::class);
        $this->reader = $reader ?: \Magento\Framework\App\ObjectManager::getInstance()
            ->create(\Magento\Framework\Module\Dir\Reader::class);
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($this->isSeller($customerid)) {
            ...
            if (empty($errors)) {
                ...
                $postData['seller_id'] = $postData['customer_id'];
                $value->addData($postData);
                $value->setIsSeller(1);
                $value->setUpdatedAt($this->_date->gmtDate());
                $value->save();
                if (isset($postData['seller_category_ids']) && !empty($postData['seller_category_ids'])) {
                ...
                if (isset($postData['flag_cpapi'])) {
                    $value->setFlagCpapi($postData['flag_cpapi']);
                }
    ...
    private function removePartner($sellerId)
    {
        ...
        foreach ($sellerProduct as $productInfo) {
            $productIdsToClear[] = $productInfo->getMageproductId();
        ...
        if (!empty($productIdsToClear)) {
            try {
                ...
                $this->mpHelper->logDataInLogger(
                    "Observer_AdminhtmlCustomerSaveAfterObserver removePartner updateAttributes : " . $e->getMessage()
                );
            }
        }
    ...
    public function assignProduct($sellerId, $productIds)
    {
        ...
        foreach ($productCollection as $product) {
            ...
            if ($flag) {
                ...
                $collection1->setMageProRowId($proid);
    ...
    public function unassignProduct($sellerId, $productIds)
    {
        ...
        if (!empty($productids)) {
            try {
                ...
                $this->mpHelper->logDataInLogger(
                    "Observer_AdminhtmlCustomerSaveAfterObserver unassignProduct : " . $e->getMessage()
                );
            }
        }
    }
