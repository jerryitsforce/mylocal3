<?php
namespace Branch8\MarketplaceProduct\Model\Import;
...
class InventoryImport
{
    ...
    public function importInventoryFromCsv($filePath)
    {
        ...
        while (($row = $stream->readCsv()) !== false) {
            ...
            try {
                ...
                if (!$this->isSellerProduct($sku, $sellerId)) {
                    $result['error'][] = "SKU $sku does not belong to current seller or is not approved.";
                    $this->logger->error('[InventoryImport] SKU ' . $sku . ' does not belong to seller ' . $sellerId);
                    continue;
                }
                if($productType === '單規') {
                    $qty = (float)$qty;
     ...
     * @return bool
     */
    private function isSellerProduct(string $sku, int $sellerId): bool
    {
        $connection = $this->resourceConnection->getConnection();
        $mpProductTable = $this->resourceConnection->getTableName('marketplace_product');
        
        $select = $connection->select()
            ->from(['mp' => $mpProductTable], ['count' => new \Zend_Db_Expr('COUNT(*)')])
            ->joinLeft(
                ['cpe' => $this->resourceConnection->getTableName('catalog_product_entity')],
                'mp.mageproduct_id = cpe.entity_id',
                []
            )
            ->where('cpe.sku = ?', $sku)
            ->where('mp.seller_id = ?', $sellerId)
            ->where('mp.status = ?', 1); // Only approved products
