...
namespace Branch8\WidgetCache\Plugin\CatalogCustom;

use Branch8\CatalogCustom\Block\Widget\CategoryList;
...
class CategoryListPlugin
{
	...
	public function aroundToHtml(CategoryList $subject, \Closure $proceed): string
	{
		...
		if ($cached !== null) {
			$this->widgetCacheHelper->logDebug('Widget cache hit for category list widget', ['cache_key' => $cacheKey]);
			return $cached;
		}
		...
		$this->widgetCacheHelper->saveWidgetContentToCache($cacheKey, $content, ['widget_cache', 'category_list_widget']);
		return $content;
	}
}
