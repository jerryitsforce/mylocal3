...
use Branch8\Blog\Block\Adminhtml\Post\Edit\Tab\Renderer\Category as CategoryOptions;
use Branch8\Blog\Block\Adminhtml\Post\Edit\Tab\Renderer\Tag as TagOptions;
use Branch8\Blog\Block\Adminhtml\Post\Edit\Tab\Renderer\Topic as TopicOptions;
...
use Branch8\Blog\Model\Config\Source\AuthorStatus;
use Branch8\Blog\Model\LikeFactory;
...
class Frontend extends Template
{
    ...
    public function getPostInfo($post)
    {
        try {
            ...
            $html           = __(
                '<i class="blog-icon blog-calendar-times"></i> %1',
            ...
            if ($author && $author->getName() && $this->helperData->showAuthorInfo()) {
                ...
                $html .= __('| <i class="blog-icon blog-user"></i> %1', $aTag);
            }

            if ($this->getCommentinPost($post)) {
                $html .= __(
                    '| <i class="blog-icon blog-comments" aria-hidden="true"></i> %1',
            ...
            if ($post->getViewTraffic()) {
                $html .= __(
                    '| <i class="blog-icon blog-traffic" aria-hidden="true"></i> %1',
                    $post->getViewTraffic()
                );
            }

            if ($couldLike > 0) {
                $html .= __('| <i class="blog-icon blog-thumbs-up" aria-hidden="true"></i> %1', $couldLike);
            }
        } catch (Exception $e) {
            $html = '';
        }
    ...
    {
        return $this->getViewFileUrl('Branch8_Blog::media/images/no-artist-image.jpg');
    }
    ...
    {
        return $this->getRequest()->isAjax() && $this->getRequest()->getParam('is_scroll');
    }
}
