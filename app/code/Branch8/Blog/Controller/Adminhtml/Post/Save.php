...
use Branch8\Blog\Model\Post as PostModel;
...
class Save extends Post
{
    ...
    public function execute()
    {
        ...
        if ($data = $this->getRequest()->getPost('post')) {
            ...
            try {
                if (empty($action) || $action === 'add') {
                    ...
                    $this->_eventManager->dispatch(
                        'branch8_blog_post_prepare_save',
                ...
                $this->_getSession()->setData('branch8_blog_post_data', false);

                if ($this->getRequest()->getParam('back')) {
                    $resultRedirect->setPath('branch8_blog/*/edit', ['id' => $post->getId(), '_current' => true]);
                } else {
            ...
            $this->_getSession()->setData('branch8_blog_post_data', $data);
    ...
    protected function prepareData($post, $data = [])
    {
        ...
        if (!$this->getRequest()->getParam('image_two')) {
            try {
                $this->imageHelper->uploadImage($data, 'image_two', Image::TEMPLATE_MEDIA_TYPE_POST, $post->getImage2());
            } catch (Exception $exception) {
                $data['image_two'] = isset($data['image_two']['value']) ? $data['image_two']['value'] : '';
            }
        } else {
            $data['image_two'] = '';
        }
        if (!$this->getRequest()->getParam('image_three')) {
            try {
                $this->imageHelper->uploadImage($data, 'image_three', Image::TEMPLATE_MEDIA_TYPE_POST, $post->getImage2());
            } catch (Exception $exception) {
                $data['image_three'] = isset($data['image_three']['value']) ? $data['image_three']['value'] : '';
            }
        } else {
            $data['image_three'] = '';
        }
