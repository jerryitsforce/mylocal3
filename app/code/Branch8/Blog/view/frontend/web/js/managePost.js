...
define([
    ...
    'uiRegistry',
    'moment',
    "Branch8_Blog/js/get-editor"
], function ($, $t, _, modal, registry, moment, editor) {
    'use strict';

    $.widget('mageplaza.BlogManagePost', {
            options: {
                deleteUrl: '',
                basePubUrl: '',
                postDatas: {},
            ...
            _create: function () {
                var self      = this,
                    htmlPopup = $('#blog-new-post-popup');

                $('.blog-new-post button').on('click', function () {
                    self._AddNewPost(self, htmlPopup);
                });

                $('.blog-post-edit').on('click', function () {
                    self._EditPost(self, this, htmlPopup);
                });

                $('.blog-post-duplicate').on('click', function () {
            ...
            _EditPost: function (self, click, htmlPopup) {
                ...
                if (htmlPopup.find('#blog_post_form [name="name"]').length > 0) {
            ...
            _setPopupFormData: function(postData, pubUrl, htmlPopup){
                _.each(postData, function (value, name) {
                    var field = htmlPopup.find('#blog_post_form [name="' + name + '"]'),
                        imageEL,
                        deleteEL,
                        date;
            ...
            _openPopup: function (options, htmlPopup, self) {
                ...
                $('#blog_post_form').trigger('contentUpdated');
    ...
    return $.mageplaza.BlogManagePost;
});
