...
namespace Branch8\CustomNotification\Model\Notification;

use Magenest\NotificationBox\Model\Notification;

class DataProvider extends \Magenest\NotificationBox\Model\Notification\DataProvider
{
    /**
     * Get data
     *
    ...
    public function getData()
    {
        ...
        foreach ($items as $model) {
            ...
            // Custom field
            if ($model->getData('customer_levels')) {
                $model->setData('customer_levels', $this->serialize->unserialize($model->getCustomerLevels()));
            }
            ...
            $data = $model->getData();
            $data['defaultTotalOneIds'] = 0;
            if ((string)$data['oneid_list']) {
                $data['defaultTotalOneIds'] = count(explode(',', (string)$data['oneid_list']));
            }

            unset($data['oneid_list']);
