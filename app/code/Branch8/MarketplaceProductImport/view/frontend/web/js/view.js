    ...
    $.widget('mpmassupload.view', {
        ...
        _create: function () {
            ...
            $(document).ready(function () {
                ...
                var selectBox = jQ("<select id=\"attribute_profile_id\" name=\"attribute_profile_id\" title=\"Dataflow Profile\" ></select>");
                jQ("<option></option>", {value: '', text: dataflowProfileMsg}).appendTo(selectBox);
                jQ(selectBox).insertAfter("#dataFlow");
                ...
                var defaultTitle = DOMPurify.sanitize(jQ(".wk-mu-options-content").html());
                var defaultContent = DOMPurify.sanitize(jQ(".wk-mu-custom-attribute").html());
                var multiSelectDefaultTitle = DOMPurify.sanitize(jQ(".wk-multi-select-options-content").html());
                var options = [];
                ...
                $(document).on('click', '#is_link_samples', function (event) {
                    var val = $(this).prop("checked");
                    if (val == true) {
                        $("#is_link_sample").after(linkSampleRow);
                    } else {
                        $("#link_sample_file").remove();
                    }
                });
                ...
                $(document).on('change', '#attribute_set', function (event) {
                    ...
                    if (id != "") {
                        ...
                        jQ(".wk-mu-custom-attribute").html(defaultContent);
                    }
                ...
                $(document).on('change', '#attribute_info', function (event) {
                    ...
                    var code = $(this).val();
                    if (code == "") {
                        jQ(".wk-mu-options-content").each(function(){
                ...
                {
                    jQ(".wk-mu-options-content").empty();
                    jQ(".wk-mu-options-content").append(defaultTitle);
                }
                function setOptions(json)
                {
                    jQ(".wk-mu-options-content").empty();
                    jQ.each(json, function (key, value) {
                        jQ(".wk-mu-options-content").append("<div class='wk-mu-options-item'>"+DOMPurify.sanitize(value)+"</div>");
                    });
                }
                function showLoader()
                {
                ...
                {
                    jQ(".wk-multi-select-options-content").empty();
                    jQ.each(json, function (key, value) {
                        jQ(".wk-multi-select-options-content").append("<div class='wk-mu-options-item'>"+DOMPurify.sanitize(value)+"</div>");
                    });
                }
                function setDefaultMultiSelectAttributeContent(multiSelectDefaultTitle)
                {
                    jQ(".wk-multi-select-options-content").empty();
                    jQ(".wk-multi-select-options-content").append(multiSelectDefaultTitle);
                }
