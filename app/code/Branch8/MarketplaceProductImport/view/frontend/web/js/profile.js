                ...
                function importProduct(count, postData)
                {
                    ...
                    jQ.ajax({
                        ...
                        success: function (response) {
                            if (response['error'] == 1) {
                                if (myData.error && myData.msg) {
                                    jQ(".fieldset").append('<div class="wk-mu-error wk-mu-box">' + myData.msg + '</div>');
                                } else {
                                    jQ(".fieldset").append(DOMPurify.sanitize(response['msg']));
                                }
                                skipCount++;
                            ...
                            $(".wk-mu-progress-bar-current").animate({width: width + "%"}, 'slow', function () {
                                if (total == 1 && skipCount == 1) {
                                    ...
                                    if (count == total) {
                                        ...
                                        postData = response['next_row_data'];
                ...
                function finishImporting(count, skipCount, postData)
                {
                    jQ.ajax({
                        ...
                        success:function (data) {
                            jQ(".fieldset").append(DOMPurify.sanitize(data['msg']));
                            $(".wk-mu-info-bar").addClass("wk-no-padding");
