...
namespace Branch8\FamilyBonusPin\Ui\DataProvider\Product;
...
class TicketRecordOverviewDataProvider extends AbstractDataProvider
{
    /**
     ...
     * @return Collection
     */
    private function getTotalCollection(): Collection
    {
        $this->getCollection()
            ->join(
                [BatchSettingModel::TABLE_NAME => BatchSettingModel::TABLE_NAME],
                'main_table.batch_setting_id = ' . BatchSettingModel::TABLE_NAME . '.setting_id'
            )
            ->addFieldToFilter(
    ...
    /**
     * 獲取各batch_code底下已賣出的serial_number總數
     * 陣列index: batch_code
