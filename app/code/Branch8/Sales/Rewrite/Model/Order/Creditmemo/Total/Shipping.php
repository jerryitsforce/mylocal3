 ...
 */
namespace  Branch8\Sales\Rewrite\Model\Order\Creditmemo\Total;
        ...
        if ($creditmemo->hasBaseShippingAmount()) {
            ...
            if ($desiredAmount < $this->priceCurrency->round($maxAllowedAmount) + 0.0001) {
                ...
                if ($desiredAmount > $maxAllowedAmount - 0.0001) {
                    ...
                    $shippingAmount = $this->priceCurrency->round($desiredAmount);
                    $baseShippingAmount = $this->priceCurrency->round($desiredAmount);
                }
                $shippingInclTax = $this->priceCurrency->round($desiredAmount);
                $baseShippingInclTax = $this->priceCurrency->round($desiredAmount);
            } else {
