...
//已送達 -> 已完成換貨
class UpdateStatusToReplaced
{

    const LOG_PATH = 'Sales/Cron/updateStatusToReplaced';
    ...
    public function execute()
    {
        $this->hotaiCoreCommon->writeLog(
            "------Start Of Cron-Update-Status-To-Replaced-----",
            self::LOG_PATH
        );
            ...
            try {
                ...
                foreach($items as $singleItem) {
                    ...
                    $status = $this->rmaActions->changeItemStatusBySalesCron(
                        $singleItem->getId(), RmaStatus::REPLACE_COMPLETE
                    );
        ...
        $this->hotaiCoreCommon->writeLog(
            "------End Of Cron-Update-Status-To-Replaced-----",
            self::LOG_PATH
        );
    }

    /**
     * getCheckOrderCollectionList 取得近 30-32 天是派車回收的單
     *
    ...
    private function getCheckOrderCollectionList()
    {
        ...
        $from = date('Y-m-d H:i:s', strtotime('-32 days', strtotime($now)));
