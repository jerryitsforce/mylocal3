...
class UpdateTicketRmaStatus
{

    const LOG_PATH = 'Sales/Cron/updateTicketRmaStatus';
    ...
    public function execute()
    {
        ...
        $this->hotaiCoreCommon->writeLog(
            "------Start Of Cron-Update-Ticket-Rma-Avail-----",
            self::LOG_PATH
        );
        ...
        foreach ($collection->getData() as $data) {
            try {
                ...
                $this->hotaiCoreCommon->writeLog(
                    "------Update Ticket Rma Status To AVALI ----",
                    self::LOG_PATH
                );
        ...
        $this->hotaiCoreCommon->writeLog(
            "-----End Of Cron-Update-Ticket-Rma-Avail----",
            self::LOG_PATH
        );
    }
    ...
    {
        $yesterday = date('Y-m-d 16:00:00', strtotime('-1 day'));
        $to = date('Y-m-d H:i:s');

        $collection = $this->orderCollectionFactory->create()
            ->addFieldToFilter('is_virtual', 1)
            ->addFieldToFilter(
                'updated_at', array('from' => $yesterday, 'to' => $to))
            ->addFieldToFilter(
                'shipping_method', ['null' => true]);
