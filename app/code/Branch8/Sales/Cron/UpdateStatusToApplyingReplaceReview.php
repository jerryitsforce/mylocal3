...
//換貨派車回收 -> 檢驗中
class UpdateStatusToApplyingReplaceReview
{

    const LOG_PATH = 'Sales/Cron/updateStatusToApplyingReplaceReview';
    ...
    public function execute()
    {
        $this->hotaiCoreCommon->writeLog(
            "------Start Of Cron-Update-Status-Applying-Replace-Review-----",
            self::LOG_PATH
        );

        $collection = $this->getReplaceShipToSupplierOrderCollectionList();
        ...
        foreach ($collection->getItems() as $item) {
            try {
                ...
                $status = $this->rmaActions->changeItemStatusBySalesCron(
                    $itemId, RmaStatus::REPLACE_REVIEW_PROCESSING
                );
     ...
     */
    private function getReplaceShipToSupplierOrderCollectionList()
    {
        ...
        $filter = [
            'from'   => $from,
            'to'     => $to,
            'status' => Status::STATUS_REPLACE_SHIP_TO_SUPPLIER,
        ];
        ...
        $collection = $this->itemCollection->create()
            ->addFieldToFilter(
                'flow_status', Status::STATUS_REPLACE_SHIP_TO_SUPPLIER
            )->addFieldToFilter(
                'item_id', $itemId
            );
