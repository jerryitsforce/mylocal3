<?php
namespace Branch8\PageBuilder\Plugin;
...
use Magento\PageBuilder\Model\Filter\Template as PageBuilderTemplate;
use Magento\PageBuilder\Plugin\Filter\TemplatePlugin;
use Psr\Log\LoggerInterface;

class TemplateFilterImage
{
    /**
     * @var ConfigInterface
     */
    ...
    /**
     * @param PageBuilderTemplate $subject
     * @param \Closure $proceed
     * @param string $result
     * @return string
     * @throws LocalizedException
     */
    public function aroundFilter(PageBuilderTemplate $subject, \Closure $proceed, string $result)
    {
        $this->domDocument = false;
        $this->scripts = [];
        $document = $this->getDomDocument($result);
        $this->generateHideFields($document);
    ...
    private function createDomDocument(string $html) : DOMDocument
    {
        ...
        try {
            ...
            if(\Branch8\HotaiCore\Helper\DebugLog::isEnable('Branch8_PageBuilder', 'exceptionlog')){
    ...
    private function generateBackgroundImageStyles(DOMDocument $document) : void
    {
        ...
        foreach ($nodes as $node) {
            ...
            $tagImage = $node->attributes->getNamedItem('data-background-tag-image');
            if ($backgroundImages->nodeValue !== '') {
                // phpcs:ignore Magento2.Functions.DiscouragedFunction
                $images = $this->json->unserialize(stripslashes($backgroundImages->nodeValue));
                if (count($images) > 0) {
                    if ($tagImage && $tagImage->nodeValue == 1) {
                        $div = $xpath->document->createElement('div');
                        $div->setAttribute('class', 'banner-img-tag-wrapper');
                        if (isset($images['desktop_image'])) {
                            if (str_contains($images['desktop_image'], '?')) $images['desktop_image'] .= '&optimize=medium';
                            else $images['desktop_image'] .= '?optimize=medium';
                            $img = $xpath->document->createElement('img');
                            $img->setAttribute('src', $images['desktop_image']);
                            $img->setAttribute('alt', 'Hotai');
                            $img->setAttribute('class', 'banner-dk');
                            $img->setAttribute('loading', 'lazy');
                            $div->appendChild($img);
                        }
                        if (isset($images['mobile_image']) && $this->getMediaQuery('mobile')) {
                            if (str_contains($images['mobile_image'], '?')) $images['mobile_image'] .= '&optimize=medium';
                            else $images['mobile_image'] .= '?optimize=medium';
                            $img = $xpath->document->createElement('img');
                            $img->setAttribute('src', $images['mobile_image']);
                            $img->setAttribute('alt', 'Hotai');
                            $img->setAttribute('class', 'banner-mb');
                            $img->setAttribute('loading', 'lazy');
                            $div->appendChild($img);
                        }
    ...
    /**
     * Generate the hide fields by removing them from the DOM
     *
     * @param DOMDocument $document
     */
    private function generateHideFields(DOMDocument $document) : void
    {
        $xpath = new DOMXPath($document);
        $nodes = $xpath->query('//*[@data-display="false"]');
        foreach ($nodes as $node) {
            $node->remove();
        }
    }
