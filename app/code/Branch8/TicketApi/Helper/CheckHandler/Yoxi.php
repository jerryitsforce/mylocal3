...
use Branch8\Yoxi\Helper\Api as YoxiApiHelper;
...
class Yoxi extends BaseHandler
{
    ...
    public function __construct(
        ...
        $this->yoxiApiHelper          = $yoxiApiHelper;
    ...
    public function handleCheckTicket(): void
    {
        ...
        // $hasValueLeft = $this->checkTicketHasValueLeft($model);

        // If YOXI ticket has no value left.
        // if (!$hasValueLeft) {
        ...
        // If YOXI ticket is useable and has value left.
        // if ($this->checkIfTicketUseable($model) && $hasValueLeft) {
    ...
    protected function checkIfTicketOverDue(Model $model): bool
    {
        ...
        // Even if YOXI ticket get "used" it can still has value left,
        // so I think we don't need to check it's "used" status here.
        // $overDueCond1 = ($status == TicketStatus::STATUS_UNUSED) && time() > strtotime($useEndTime);
        $overDueCond1 = time() > strtotime($useEndTime);
        $overDueCond2 = $model->getStatus() == TicketStatus::STATUS_OVER_DUE;

        return $overDueCond1 || $overDueCond2;
    }

    protected function checkTicketHasValueLeft(Model $model): bool
    {
        $serialNumber = $model->getSerialNumber();

        $apiResponse       = $this->yoxiApiHelper->requestApiGetCouponList([$serialNumber]);
        $serialNumberArray = $this->yoxiApiHelper->getDecryptContentFromResponse($apiResponse);

        $totalCount      = $serialNumberArray["CouponList"][0]["TotalCount"];
        $usedCount       = $serialNumberArray["CouponList"][0]["UsedCount"];
        $invalidateCount = $serialNumberArray["CouponList"][0]["InvalidateCount"];

        return $totalCount > ($usedCount + $invalidateCount);
    }
    ...
    //     $cond1 = ($status == TicketStatus::STATUS_UNUSED) || ($status == TicketStatus::STATUS_USED);
