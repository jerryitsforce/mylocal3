...
class Edenred
{
    const LOG_FOLDER_NAME = "TicketApi/Api/NotifyUsed/Edenred";
    ...
    {
        $uniqueId = $this->apiHelper->getSerialNo();
        $model    = $this->modelRepository->getRecordByVoucherNo($uniqueId);
        ...
        if (!$model) {
            ...
            $this->returnData->setSerialNo($uniqueId);
        ...
        if ($this->checkIfTicketUnableToUseYet($model)) {
            $memoArray = [
                ...
                "Decrypt Request" => $this->apiHelper->getDecryptArray(),
                "Use Start Time"  => $model->getEdenredExpireStartDate(),
                "Datetime"        => date("Y-m-d H:i:s"),
                "Timestamp"       => time(),
                "IP"              => $this->apiHelper->getIp(),
            ];
        ...
        if ($this->checkIfTicketOverDue($model)) {
            $memoArray = [
                ...
                "Decrypt Request" => $this->apiHelper->getDecryptArray(),
                "Use End Time"    => $model->getEdenredExpireEndDate(),
                "Datetime"        => date("Y-m-d H:i:s"),
                "Timestamp"       => time(),
                "IP"              => $this->apiHelper->getIp(),
            ];
        ...
        $this->returnData->setUsedTransactionNo($this->apiHelper->getUsedTransactionNo());
        $this->returnData->setSerialNo($model->getEdenredVoucherNo());
        $this->returnData->setProductNo("");
        $this->returnData->setProductNameM($orderItem->getName());
        $this->returnData->setProductNameS("");
    ...
    /**
     * 判斷票券狀態是否已處於"已使用"
     * @param \Branch8\Edenred\Model\EdenredTicketRecord $model
     * @return bool
     */
    ...
    protected function updateTicketTableAndCustomerTicketTableInTransaction(Model $model): Model
    {
        ...
        try {
            ...
            $whereUpdate = [
                CustomerTicket::TYPE . ' = ?' => VirtualProductType::TYPE_EDENRED_TICKET,
