...
use Magento\Staging\Model\Entity\Update\Action\Save\SaveAction as StagingSaveAction;
...
class SaveAction
{
    ...
    public function aroundExecute(
        ...
        if ($this->state->getAreaCode() == Area::AREA_CRONTAB
            ...
            || in_array($controller, ['catalogstaging', 'cmsstaging', 'catalogrulestaging', 'salesrulestaging', 'promocatalog'])) {
            return $proceed($params);
        }
        $this->validator->validateParams($params);
        if (!empty($params['entityData']['status']) && empty($params['entityData']['product']['status'])) {
            $params['entityData']['product']['status'] = $params['entityData']['status'];
        }
        $stagingId = $params['stagingData']['update_id'] ?? null;
        $productId = $params['entityData']['id'] ?? $params['entityId'] ?? null;
        ...
        $createdFrom = CreatedFrom::CREATED_FROM_SCHEDULE;
        if (isset($entityData['created_from'])) {
            $createdFrom = $entityData['created_from'];
            unset($entityData['created_from']);
        }
        ...
        $returnArr = $this->storeChangedData->execute($entityData, $productId, $sellerId, $createdFrom);
        ...
        if ($shouldSave) {
            if (!$stagingId && !$skipLog) {
                ...
                if ($stagingId) {
                    ...
                    $data['staging']['update_id'] = $stagingId;
            ...
            $productVerData = $this->productVersionDataRepository->get((int)$logId, true);
            $productVerDataInformation = $this->serializer->unserialize($productVerData->getInformation());
            /** case change start time of schedule */
            if(isset($productVerDataInformation['schedule_change_start_time']) && $productVerDataInformation['schedule_change_start_time'] 
                && isset($productVerDataInformation['rid'])){
                
                $_conn = $this->resourceConnection->getConnection();
                $_select = 'select created_in from catalog_product_entity where row_id='.$productVerDataInformation['rid'];
                $rowCreatedIn = $_conn->fetchOne($_select);

                $dataInformation = $this->serializer->unserialize($productVerData->getInformation());
                $dataInformation['staging']['update_id'] = $rowCreatedIn;
                $dataInformation['update_id'] = $rowCreatedIn;
                $productVerData->setInformation($this->serializer->serialize($dataInformation));
                $this->productVersionDataRepository->save($productVerData);
            }
            return $result;
        }
