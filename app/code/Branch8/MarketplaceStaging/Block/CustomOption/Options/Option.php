...
class Option extends Template
{
     ...
     * @var string
     */
    protected $_template = 'Branch8_MarketplaceStaging::catalog/product/edit/options/option.phtml';
    ...
    {
        parent::_construct();
        $this->setTemplate('Branch8_MarketplaceStaging::catalog/product/edit/options/option.phtml');
    ...
    public function getTypeSelectHtml()
    {
        ...
        $select = $this->getLayout()->createBlock(
            ...
            [
                ...
                'class' => 'select select-product-option-type required-option-select disabled'
            ]
        )->setName(
    ...
    public function getOptionValues()
    {
        ...
        if (!$this->_values || $this->getIgnoreCaching()) {
            ...
            foreach ($optionsArr as $option) {
                ...
                $this->setItemCount($option->getOptionId()??$j);

                $value = [];

                $value['id'] = $option->getOptionId() ?? $j;
                $value['item_count'] = $this->getItemCount();
                $value['option_id'] = $option->getOptionId() ?? $j;
                if (!$option->getOptionId()) $value['record_id'] = $j;
                ...
                if ($option->getGroupByType() == ProductCustomOptionInterface::OPTION_GROUP_SELECT) {
                    ...
                    if (isset($option['values'])) {
                        foreach ($option['values'] as $_value) {
                            if(!array_key_exists('title', $_value)){
                                continue;
                            }
                            /* @var $_value \Magento\Catalog\Model\Product\Option\Value */
                            $optionTypeId = $i;
                            if (isset($_value['option_type_id']) && $_value['option_type_id']) {
                                $optionTypeId = $_value['option_type_id'];
                            }
                            if(isset($_value['is_bought']) && $_value['is_bought'] == 1){
                                $value['show_delete_button'] = 0;
                            }
                            $price = 0;
                            if (isset($_value['price']) && $_value['price']) {
                                $price = $_value['price'];
                            }
                            $priceType = 'fixed';
                            if (isset($_value['price_type']) && $_value['price_type']) {
                                $priceType = $_value['price_type'];
                            }
                            $value['optionValues'][$i] = [
                                'item_count' => max($itemCount, $optionTypeId),
                                'option_id' => $option->getOptionId() ?? $j,
                                ...
                                'price_type' => $showPrice ? $priceType ?? 'fixed' : 0,
                                'sku' => $_value['sku'],
                                'is_bought' => $_value['is_bought'],
                                'is_visible' => $_value['is_visible'],
                                'sort_order' => $_value['sort_order'],
                                'title_option' => $_value['title'],
                            ];
                            if (!$option->getOptionId()) $value['optionValues'][$i]['add_new'] = $j;
                            $i++;
                        }
                    } else {
