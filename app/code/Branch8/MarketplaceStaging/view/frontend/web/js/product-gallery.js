...
define([
     ...
     */
    $.widget('mage.stageProductGallery', {
        options: {
            imageSelector: '[data-role=staging-image]',
            template: '[data-template=staging-image]',
            types: null,
            initialized: false
        },
        ...
        _bind: function () {
            var events = {
                ...
                resort: '_resort',
                'mouseup [data-role=staging-delete-button]': function (event) {
                    var $imageContainer;

                    event.preventDefault();
                    $imageContainer = $(event.currentTarget).closest(this.options.imageSelector);
                    this.element.find('[data-role=staging-dialog]').trigger('close');
                    this.element.trigger('removeItem', $imageContainer.data('imageData'));
                },
                'mouseup [data-role=staging-make-base-button]': function (event) {
                    var $imageContainer,
                        imageData;
        ...
        setBase: function (imageData) {
            ...
            $.each(sameImages, $.proxy(function (index, image) {
                ...
                if (isImageOpened) {
                    ...
                    this.element.find('[data-role=staging-type-selector]').prop({
                        'checked': true
                    });
                }
            }, this));
    ...
    $.widget('mage.stageProductGallery', $.mage.stageProductGallery, {
        options: {
        ...
        _showDialog: function (imageData) {
            ...
            dialogElement
                ...
                .on('open', $.proxy(function (event) {
                    $(event.target)
                        .find('[data-role=staging-type-selector]')
            ...
            dialogElement.on('change', '[data-role=staging-type-selector]', function () {
                var parent = $(this).closest('.item'),
                    selectedClass = 'selected';
                parent.toggleClass(selectedClass, $(this).prop('checked'));
            });
            dialogElement.on('change', '[data-role=staging-type-selector]', $.proxy(this._notifyType, this));
            dialogElement.on('change', '[data-role=staging-visibility-trigger]', $.proxy(function (e) {
        ...
        _changeVisibility: function (event, imageData) {
            ...
            var $imageContainer = $checkbox.closest('[data-role=staging-dialog]').data('imageContainer');
