define([
    ...
    return Element.extend({
        ...
        reload: function (options) {
            if (this.params.namespace=='marketplacestaging_related_product_listing') {
                this.params.current_product_id = $("input[name='product_id']").val();
                this.params.update_id = $("input[name='update_id']").val();
            } else if (this.params.namespace=='marketplacestaging_crosssell_product_listing') {
                this.params.current_product_id = $("input[name='product_id']").val();
                this.params.update_id = $("input[name='update_id']").val();
            } else if (this.params.namespace=='marketplacestaging_upsell_product_listing') {
                this.params.current_product_id = $("input[name='product_id']").val();
                this.params.update_id = $("input[name='update_id']").val();
            } else if (this.params.namespace=='marketplacestaging_upcoming_grid') {
                if (!this.params.id) this.params.id = $("input[name='product_id']").val();
            ...
            } /*else if (this.params.namespace=='marketplace_product_attributes_listing') {
                let arraySelected = [];
                $(".selected-attr input:checkbox:checked").each(function(){
                    arraySelected.push($(this).val());
                });
                this.params.attribute_selected = arraySelected;
            } else if (this.params.namespace=='marketplacestaging_product_attributes_listing') {
                let arraySelected = [];
                $(".stage-selected-attr input:checkbox:checked").each(function(){
                    arraySelected.push($(this).val());
                });
                this.params.attribute_selected = arraySelected;
            }*/
