...
class Category extends \Magento\Backend\Block\Widget\Form\Generic implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
    ...
    {
        parent::_construct();
        $this->setTemplate('Branch8_Mmegamenu::category.phtml');
    }
    ...
    public function getCategory(){
        ...
        $category = $this->getModel('Magento\Catalog\Model\CategoryRepository')->get($categoryId);
        return $category;
    }
    ...
    public function getTreeCategory($category, $parent, $ids = array()){
        if ($this->getRequest()->getParam('id')) {
            $megamenu = $this->getModel('Branch8\Mmegamenu\Model\Mmegamenu')->load($this->getRequest()->getParam('id'));
        ...
        if($childrenCount>0){
            foreach ($children as $child) {
                ...
                $_child = $this->getModel('Magento\Catalog\Model\CategoryRepository')->get($child->getId());
