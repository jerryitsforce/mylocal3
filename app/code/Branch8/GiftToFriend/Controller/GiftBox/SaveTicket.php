 ...
 *
 */
class SaveTicket extends \Magento\Framework\App\Action\Action implements HttpPostActionInterface
{
    /**
    ...
    public function execute()
    {
        ...
        try {
            ...
            if (isset($postDetail['order_id'])) {
                $postDetail['parent_order'] = $postDetail['order_id'];
            }
            $ticket->setData($postDetail);

            $customer = $this->customerSession->getCustomer();
            if($customer && $customer->getId()){
                $ticket->populateWithCustomerData(
                    $customer
                );
            }
            ...
            $cateId = $this->scopeConfig->getValue('gift_order/customer_service_form/gift_category');
            $ticket->setData('category_id', $cateId);
            ...
            if($customer && $customer->getId()){
                ...
                $customerData = ['name' => $postDetail['customer_name'], 'email' => $postDetail['email']];
                $message = $this->createMessageFromPostAction->createForGuest(
                    $ticket,
                    $customerData,
                    $postDetail
                );
                $this->lastReplyAction->saveByGuest($ticket,
                    $customerData
                );
            }
            /**
