...
define([
    ...
    'mage/template',
    'plugins/DOMPurify'
], function($, $t, modal, url, ko, alert, mageTemplate, DOMPurify) {
    'use strict';
    return function(option) {
        ...
        $('body').on('click', '[data-index=wkswatches]', function() {
            var optionValuesTitles = [];
            var swatchDatas, swatchTitles = [];
            var str = $('#save-button').data('wk-variation');
            if (typeof str == "undefined" || str == "") {
                alert({ 'content': $t("No option available.") });
                return;
            }
            try {
                swatchDatas = JSON.parse(str);
            } catch (e) {
        ...
        $('body').on('click', '[data-index=wkvariations]', function() {
            if (window.useManageVariant2) {
                return;
            }
            var mess = buildWkSwatch();
            if(mess !== ""){
                alert({ 'content': mess });
                return;
            }
            ...
            $.each(datas, function(key, data) {
                if (data.type == "radio" || data.type == "drop-down" || data.type == "drop_down") {
                    ...
                    $.each(data.values, function(index, value) {
                        ...
                        oldValueArr.push(value.default_title);
            ...
            variationTemp = progressTmpl({
                data: {
                    ...
                    weightDisabled: !parseInt($("[name='product[product_has_weight]']").val())
                }
            })
        ...
        $("body").on('change', '#follow_simple_sku_cost_setting', function() {
            if ($(this).is(":checked")) {
                var cost_setting = $('input[name="cost_setting_radio"]:checked').val();
