...
class DownloadDetailFile extends Column
{
    /** @var UrlInterface */
                ...
                if (isset($item['record_id'])) {
                    $item[$this->getData('name')] = [
                        'download_original_file' => [
                            'href'   => $this->getLinkUrlForColumn($item['record_id'], CommonHelper::FILE_TYPE_ORIGINAL_DETAIL_FILE),
                            'target' => '_blank',
                            'label'  => __('Original File')
                        ],
                        'download_modified_file' => [
                            'href'   => $this->getLinkUrlForColumn($item['record_id'], CommonHelper::FILE_TYPE_MODIFIED_DETAIL_FILE),
                            'target' => '_blank',
    ...
    protected function getLinkUrlForColumn(int $record_id, string $file_type): string
    {
        return $this->urlBuilder->getUrl(
            'hifi_sales_report/download/detailfile',
            [
                'record_id' => $record_id,
                'file_type' => $file_type,
            ]
