...
class DetailFile extends Action
{
    ...
    protected function initParameters(): void
    {
        ...
        $this->currentCsvData = $this->commonHelper->getFileContent(
            CommonHelper::FILE_TYPE_ORIGINAL_DETAIL_FILE,
            $this->record->getOriginalDetailFileName()
        );
    ...
    protected function deleteCurrentModifiedFile()
    {
        ...
        $this->record->setModifiedDetailFileName("");
    ...
    {
        $fileName         = $this->record->getId() . "_" . date("Ymd_His") . "_modified_detail.csv";
        $filepath         = CommonHelper::FILE_FOLDER_PATH_MODIFIED_DETAIL_FILE . "/{$fileName}";
    ...
    protected function updateRecord()
    {
        $memo = $this->hotaiCoreCommonHelper->prepareMemoStringForUpdate(
            $this->record->getMemo(),
            [
                "Title"            => "HIFI Sales Report Detail File Upload.",
        ...
        $this->record->setModifiedDetailFileName($this->uploadFileName);
