...
class FilteredMainFile extends Action
{
    ...
    public function execute()
    {
        ...
        $fileName           = $this->subRecord->getId() . "_" . time() . "_主檔_{$invoiceStatusLabel}.csv";
    ...
    protected function getFinalCsvDataForDownload(): array
    {
        $mainFileContent = $this->commonHelper->getFileContent(
            ...
            $this->mainRecord->getModifiedMainFileName()
        );

        return $this->commonHelper->getFilteredMainFileContent(
            $mainFileContent,
            $this->subRecord
        );
    }
