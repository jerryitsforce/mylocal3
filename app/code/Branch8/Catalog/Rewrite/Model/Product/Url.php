...
namespace Branch8\Catalog\Rewrite\Model\Product;
...
{
    const PDP_PREFIX = 'catalog/seo/pdp_prefix';
    /**
     * URL instance
     *
     ...
     */
    public function aroundGetUrl($subject, $process, \Magento\Catalog\Model\Product $product, $params = [])
    {
        $routePath = '';
        $routeParams = $params;
        ...
        $pdpPrefix = (string)$this->scopeConfig->getValue(self::PDP_PREFIX);
        if(trim($pdpPrefix) != ''){
            $requestPath = $pdpPrefix.$requestPath;
        }else {
