...
class Hold extends ParentOrder implements HttpPostActionInterface
{
    /**
     ...
     * @see _isAllowed()
     */
    const ADMIN_RESOURCE = 'Branch8_MarketPlaceParentOrderAdminUi::hold';
    ...
    public function execute()
    {
        ...
        if (!$this->isValidPostRequest()
            || empty($parentOrder)
            || !$parentOrder->getId()
            || !$this->parentOrderManagement->canHold($parentOrder)
        ) {
            $this->messageManager->addErrorMessage(__('You have not hold this order.'));
            return $resultRedirect->setPath('sales/*/');
        }
        try {
            $this->parentOrderManagement->hold($parentOrder);
            $this->messageManager->addSuccessMessage(__('You hold the order.'));
        } catch (\Magento\Framework\Exception\LocalizedException $e) {
            $this->messageManager->addErrorMessage($e->getMessage());
        } catch (\Exception $e) {
            $this->messageManager->addErrorMessage(__('You have not hold the item.'));
