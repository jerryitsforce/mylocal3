...
namespace Branch8\MarketPlaceParentOrderAdminUi\Block\Adminhtml\Edit\Tab\CommentHistory;

use Branch8\MarketPlaceParentOrder\Model\ParentOrder;
use Magento\Sales\Model\Order;

class Full extends \Magento\Backend\Block\Template /*implements \Magento\Backend\Block\Widget\Tab\TabInterface*/
{
    /**
     * Core registry
     *
    ...
    {
        $parentId = $this->getRequest()->getParam('pid', 0);
        $parentOrder = $this->parentOrderFactory->create()->load($parentId);
        return $parentOrder;
    }
    ...
    public function getFullHistory()
    {
        ...
        foreach ($parentOrder->getSubOrders() as $order) {
            $allHistory = array_merge($this->getSubOrderHistory($order));
        }
        usort($allHistory, [__CLASS__, 'sortHistoryByTimestamp']);
        return $allHistory;
    }
     ...
     * @return array
     */
    private function getSubOrderHistory(Order $order)
    {
        $history = [];
