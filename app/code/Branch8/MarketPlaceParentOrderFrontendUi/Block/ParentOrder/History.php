...
class History extends \Magento\Framework\View\Element\Template {
    ...
    {
        parent::_construct();
        $this->pageConfig->getTitle()->set(__('Order Management'));
    }
    ...
    public function renderShippingInfo($order)
    {
        ...
        $showAddress = true;
        if($this->isGiftOrder($order) && !$this->isGiftOrderConfirmed($order)) {
            $showAddress = false;
        }
        return $order->getShippingDescription() . ($addressString && $showAddress ? " ($addressString)" : '');
    }
    ...
    public function isPendingPaymentOrderAutoCancelled($order)
    {
        return $order->getStatus() === Order::STATE_CANCELED && (bool) $order->getData('is_auto_cancelled') && !$this->getIsPaid($order);
    }
    ...
    {
        return (bool) $order->getData('is_gift_order') && $order->getData('is_gift_confirmed');
    }
    ...
    public function getOrders()
    {
        ...
        if ($fromDate) {
            ...
            $collection->addFieldToFilter('detail.created_at', ['gteq' => $fromDate]);
        }

        $toDate = $this->getRequest()->getParam('to_date');
        if ($toDate) {
            //format date 2023-02 with last day of month
            $toDate = date('Y-m-t', strtotime($toDate));
            $collection->addFieldToFilter('detail.created_at', ['lteq' => $toDate]);
        }

        $parentId = $this->getRequest()->getParam('parent_id');
        if ($parentId) {
            $collection->addFieldToFilter('detail.parent_id', $parentId);
        }

        $searchQuery = $this->getSearchQuery();
        if ($searchQuery) {
            //if search query has No. in "No.24092809_0HG30077401" we will remove No.
            $searchQuery = preg_replace('/No\./', '', $searchQuery);
            $subOrderCollection = $this->subOrderCollectionFactory->create();
            $subOrderCollection->addFieldToSelect("entity_id");
            

            //Search Increment id first
            $subOrderCollection->getSelect()
                ->distinct()
                ->where('main_table.increment_id = ?', $searchQuery)
                ->where('main_table.customer_id = ?', $customerId)
                ->group('main_table.entity_id');
            
            
            //Search Item name
            if($subOrderCollection->getSize() == 0) {
                $subOrderCollection = $this->subOrderCollectionFactory->create();
                $subOrderCollection->addFieldToSelect("entity_id");
                ...
                $subOrderCollection->addFieldToFilter('si.name', ['like' => '%'.$searchQuery.'%']);
            }


            if ($subOrderCollection->getSize() > 0) {
                $childIds = [];
                foreach ($subOrderCollection as $item) {
                    $childIds[] = $item->getId();
                }

                $parentOrderIds = $this->getParentId($childIds);

                if (!empty($parentOrderIds)) {
                    $collection->addFieldToFilter(
                        ['detail.parent_id', 'detail.increment_id'],
                        [['in' => $parentOrderIds], ['eq' => $searchQuery]]
                    );
                }
        ...
        if (!empty($statuses) && !in_array('all', $statuses)) {
            if(in_array(Status::STATUS_COMPLETE, $statuses)) {
                $collection->addFieldToFilter('detail.status', ['in' => $statuses]);
                $collection->addFieldToFilter(
                    'detail.rma_status',[['null' => true], ['eq' => Status::STATUS_RMA_COMPLETED]]
                );
            } else if(in_array(Status::STATUS_CANCELED, $statuses)) {
                $collection->addFieldToFilter('detail.status', ['in' => $statuses]);
            } else {
                $collection->addFieldToFilter('detail.status', ['nin' => [Status::STATUS_CANCELED, Status::STATUS_COMPLETE]]);
                $collection->addFieldToFilter(
                    'detail.rma_status',[['null' => true], ['neq' => Status::STATUS_RMA_COMPLETED]]
                );
                // still missing status=complete and rma_status=null
            }
        } else {
            $collection->addFieldToFilter(
                'detail.status',
        ...
        // $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/order_sql.log');
        // $logger = new \Zend_Log();
        // $logger->addWriter($writer);
        // $logger->info('-----');
        ...
        // $logger->info(print_r($this->orders->getSelect()->__toString(), true));
    ...
    public function getIsPaid($order)
    {
        ...
        $parentChildFirstSelect = $connection->select()
        ->from($connection->getTableName('sales_parent_order_children'), ['children_id'])
        ->where('parent_id = ?', $order->getParentId());

        $firstSubOrderId = $connection->fetchOne($parentChildFirstSelect);
        
        $select = $connection->select()
            ->from($connection->getTableName('sales_order'), ['is_paid'])
            ->where('entity_id = ?', $firstSubOrderId);
...
//        }
    ...
    public function canShowMobileLoadMore()
    {
        return $this->getOrders()->getLastPageNumber() > 1;
    }
    ...
    public function getPagerHtml()
    {
        ...
        if (!$this->pager) {
            $this->pager = $this->getLayout()->createBlock(
                Pager::class,
                $pagerBlockName
            )->setTemplate('Branch8_MarketPlaceParentOrderFrontendUi::parent_order/page/pager.phtml');
