...
class SaveTrackingNumber extends \Webkul\Marketplace\Controller\Order{
    /**
     * @var HelperData
     */
    ...
    public function execute(){
        ...
        try {
            ...
            $shipment = $this->_shipment->load($request->getParam('smid'));
            if($shipment->getOrder()->getId() != $order->getId()){
                $returnData = [
                    'success' => 0,
                    'msg' => __('Your input data does not match.')
                ];
                return $jsonResult->setData($returnData);
            }
            ...
            $this->shipmentHelper->addTrackingNumberForShipment($shipment->getId(), $request->getParam('carrierCode'), $request->getParam('carrierTitle'), $request->getParam('trackingNum'));
            //reload popup
            ...
            $this->marketplaceLogger->logException('SaveTrackingNumber', $exception, [
