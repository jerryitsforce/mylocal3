...
class View extends \Webkul\Marketplace\Block\Order\View
{
    ...
    private const XML_SELLER_CAN_CREATE_RMA_PATH = 'mprmasystem/settings/seller_can_create_rma';
    /**
    ...
    */
     ...
     * @return null|string
     */
    public function getFormattedAddress(Address $address = null)
    {
        if(!$address){
            return '';
        }
    ...
    public function isOrderCanShip($order)
    {
        ...
        $isStatusValidToShip = in_array($order->getStatus(), \Branch8\Sales\ViewModel\StatusResolver::shippingStatues());
        if (!$isStatusValidToShip) {
            return false;
        }
    ...
    public function canCreateRma($order) {
        $cancreatRma = (bool)$this->_scopeConfig->getValue(self::XML_SELLER_CAN_CREATE_RMA_PATH);
        return $cancreatRma && in_array(
            $order->getStatus(),
            [
     ...
     * @return bool
     */
    public function canFailedDelivery($order) {
        return in_array(
            $order->getStatus(),
            [
                Status::STATUS_ARRIVED,
