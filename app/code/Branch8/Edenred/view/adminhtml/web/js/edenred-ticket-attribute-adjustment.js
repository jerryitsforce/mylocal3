require([
    ...
    const module_attribute_set_name = 'ticket_edenred';
    const module_virtual_product_type = "3";

    const module_reset_display_serial_number_value = 1;
    const module_allow_display_serial_number_value = [1];

    const module_reset_display_barcode_value = 1;
    const module_allow_display_barcode_value = [1];

    jQuery(document).ajaxComplete(function () {
        // 調整"是否顯示序號"(app/code/Branch8/HotaiCore/Model/Product/DisplaySerialNumber.php)的選項
        $do.get('select[name="product[display_serial_number]"]', function (elem) {
            ...
            if (!module_allow_display_barcode_value.includes(parseInt(current_value))) {
                $(elem).val(module_reset_display_serial_number_value).change();
            }

            $options.each(function () {
                let $option = $(this);
                if (!module_allow_display_serial_number_value.includes(parseInt($option.val()))) {
