...
class EditParentOrderVerify implements VerifyQuoteActionInterface
{
    ...
    public function verify(\Magento\Backend\Model\Session\Quote $quoteSession, \Magento\Sales\Model\AdminOrder\Create $adminOrderModel)
    {
        ...
        if (($id = $quoteSession->getEditParentOrderFrom())
    ...
    {
        $result = $this->parentOrderManagement->canEdit($parentOrder);
        $error = $result === false ? __('Can not edit parent order')->render() : '';
